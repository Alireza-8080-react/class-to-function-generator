...
import { lifecycleMethodNames } from "../../constants";

export const extractLifecycleHooks = (
  j: JSCodeshift,
  root: Collection<any>
) => {
  ...
  lifecycleMethodNames.forEach((methodName) => {
    // Find and process methods
    root
      .find(j.MethodDefinition, { key: { name: methodName } })
      .forEach((method) => {
  ...
  if (lifecycleMethods.componentWillUnmount.size > 0) {
    const unmountStatements = Array.from(
      ...
      .forEach((method) => {
        ...
        if (params.length > 0) {
          ...
          const cleanedUpdateStatements = updateStatements.map((statement) => {
            ...
            ifStatements.forEach((ifStatement) => {
              ...
              blockBody.forEach((bodyStatement) => {
                lifecycleMethods.componentDidUpdate.add(
                  j(bodyStatement).toSource()
                );
              });
              j(ifStatement).replaceWith(
                blockBody.map((bodyStatement) => j(bodyStatement).toSource())
              );
            });
          ...
          updateStatements.forEach((stmt) => {
            const propMatches = stmt.match(
              new RegExp(`${firstArgName}\\.(\\w+)`, "g")
            );
            propMatches &&
