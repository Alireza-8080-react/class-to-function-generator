...
const handleImports = (j: JSCodeshift, root: Collection<any>, needsState: boolean, needsEffect: boolean, needsRef: boolean) => {
    ...
    if (importSpecifiers.length > 0) {
        if (existingImport.size() > 0) {
            ...
            const newImport = j.importDeclaration(importSpecifiers, j.literal('react'));
