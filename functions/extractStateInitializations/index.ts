import { ClassDeclaration, Collection, Identifier, JSCodeshift, ObjectExpression, ObjectProperty } from 'jscodeshift';

export const extractStateInitializations = (j: JSCodeshift, classPath: Collection<ClassDeclaration>) => {
    const stateInitializations: { key: string; value: any }[] = [];

    // Find state initialization in the constructor
    classPath.find(j.MethodDefinition, { kind: 'constructor' }).forEach((constructorPath) => {
        j(constructorPath)
            ...
            .forEach((stateAssignmentPath) => {
                const typedStateAssignmentRight = stateAssignmentPath.node.right as ObjectExpression;
                typedStateAssignmentRight.properties.forEach((property) => {
                    ...
                    stateInitializations.push({ key: typedKey.name, value: typedProperty.value });
                });
        ...
        .forEach((statePropertyPath) => {
            ...
            typedStateValue.properties.forEach((property) => {
                ...
                if (typedProperty.key.type === 'Identifier') {
                    stateInitializations.push({ key: typedProperty.key.name, value: typedProperty.value });
                }
            });
    ...
    return stateInitializations.map(
        ({ key, value }) => `const [${key}, set${key.charAt(0).toUpperCase() + key.slice(1)}] = useState(${j(value).toSource()});`
    );
};
